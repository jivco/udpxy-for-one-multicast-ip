...
udp_relay( int sockfd, struct server_ctx* ctx )
{
    ...
    TRACE( (void)tmfprintf( g_flog, "udp_relay : new_socket=[%d] param=[%s]\n",
                        sockfd, ctx->rq.param) );
    (void) tmfprintf( g_flog, "jivco params web [%s]\n", ctx->rq.param );
    (void) tmfprintf( g_flog, "jivco params console [%s]\n", mcast_tvaddr );
    //jivconew
    ctx->rq.param[0] = '\0';
    strcat(ctx->rq.param,mcast_tvaddr);
    do {
        ...
        */
        ...
        if( NULL != g_uopt.srcfile ) {
            ...
            if (0 == rc ) {
              ...
              */
                rc = setup_mcast_listener( &addr, mifaddr, &msockfd,(g_uopt.nosync_sbuf ? 0 : rcvbuf_len) );
                srcfd = msockfd;
            }
        }
        if( 0 != rc ) break;
 ...
 * */
#ifdef UDPXY_FILEIO
    static const char UDPXY_OPTMASK[] = "TvSa:l:p:m:c:B:n:R:r:w:H:M:J:";
#else
    static const char UDPXY_OPTMASK[] = "TvSa:l:p:m:c:B:n:R:H:M:J:";
#endif
/*jivco*/
    ...
    while( (0 == rc) && (-1 != (ch = getopt(argc, argv, UDPXY_OPTMASK))) ) {
        switch( ch ) {
            ...
            case 'J': /*
                      rc = get_ipv4_address( optarg, mcast_tvaddr, sizeof(mcast_tvaddr) );
                      if( 0 != rc ) {
                        (void) fprintf( stderr, "Invalid TV address: [%s]\n",
                                        optarg );
                          rc = ERR_PARAM;
                      }
                      */
                      (void) fprintf( stderr, "multicast ip address: [%s]\n",optarg );
                      strcat(mcast_tvaddr, optarg);
                      //mcast_tvaddr = *optarg;
                      break;
